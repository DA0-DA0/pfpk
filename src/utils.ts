...
export const secp256k1PublicKeyToBech32Address = (hexPublicKey: string, bech32Prefix: string): string => {
...
export const verifySecp256k1Signature = async (hexPublicKey: string, message: Uint8Array, base64DerSignature: string): Promise<boolean> => {
  const publicKeyData = fromHex(hexPublicKey);
  const signature = Secp256k1Signature.fromDer(fromBase64(base64DerSignature));

  return await Secp256k1.verifySignature(signature, message, publicKeyData);
};
